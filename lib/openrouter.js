...
// Clamped to <=220s: OpenRouter is the longest leg, and under Vercel's hard 300s ceiling the effective
// timeout must leave room for the remaining legs (Resend send + owner-alert fallback). An unbounded env
// override could push the chain past 300s and get the instance killed before the owner is alerted. (Codex)
const OPENROUTER_TIMEOUT_CEILING_MS = 220000;
const DEFAULT_TIMEOUT_MS = Math.min(
  Number(optionalEnv("OPENROUTER_TIMEOUT_MS")) || 150000,
